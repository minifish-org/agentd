...
test "$("$root/scripts/release-version.sh" v0.1.0-alpha.1)" = "0.1.0-alpha.1"
! "$root/scripts/release-version.sh" v0.1.0-alpha.2 >/dev/null 2>&1
...
grep -q 'bge-reranker-v2-m3-ONNX' "$root/scripts/fetch-reranker-model.sh"
grep -q 'model_int8.onnx' "$root/scripts/fetch-reranker-model.sh"
...
test -f "$root/docs/releases/v0.1.0-alpha.1.md"
test -f "$root/deny.toml"
test -f "$root/.github/workflows/security.yml"
test -f "$root/.github/workflows/release-check.yml"
...
grep -q 'Run real embedding smoke test' "$root/.github/workflows/release-check.yml"
grep -q 'Run real reranker smoke test' "$root/.github/workflows/release-check.yml"
grep -q 'Build and inspect release image' "$root/.github/workflows/release-check.yml"
